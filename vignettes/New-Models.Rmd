...
However, goodness-of-fit-test rejects this model
...
Note that also in this simple case the standard diagnostic plots
also reveal that this model is not a sufficient.
...
we can do a wild bootstrap using that uses only the predictions
...
This class generates as many observations
...
of *GOF_lm/glm/_trainer*. The only difference is that
we again store the data with the fit because *nlsLM()*
is doesn't do this for us.
...
is used during refitting resampled data. So fitting
resampled data and fitting the original data is slightly
different. To circumvent this problem we can reimplement
the the corresponding interface as follows
