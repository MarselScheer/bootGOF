...
In order to apply the Goodness-of-Fit test one has to implement
the three interfaces *GOF_model_info_extractor*, *GOF_model_simulator*,
*GOF_model_trainer*. 

In order to illustrate the process we are gonna do that for a model
that was fitted using MASS::glm.nb. First we generate a data set
...
function manually in order to use the Goodness-of-Fit test, we hope 
...
head(d)
```
This are new $y$s generated according to the fitted
...
Hence, it is recommended/necessary to provide an implementation
